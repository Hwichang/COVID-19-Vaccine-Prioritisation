...
init_pi = matrix(0, nrow =3, ncol=16)
...
init_pi[2,] = sapply(1:16, function(x) { max(rgamma(1,0.001,rate = 0.001),1e-300)})  


pi_matrix = matrix(0, nrow = 400,ncol = 16)
...
############################ p(t)#############################
p_unit = function(t){
  res = (pi_matrix[(t-1),]/(japan_age_group*100)) * (contact_matrix%*%I_total[t-1,])
  return(res)
}
...
tilde_p_unit = function(t){
  res = (pi_matrix[(t-1),]/(japan_age_group*100)) * (contact_matrix%*%I_total_new[t-1,])
  return(res)
}
