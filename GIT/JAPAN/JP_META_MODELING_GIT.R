...
pi_list = as.matrix(read.csv('pi_list_japan_210302.csv',header=T)[,2:17])
#pi_list = as.matrix(read.csv('pi_list_japan_asym_0.04.csv',header=T)[,2:17])
#pi_list = as.matrix(read.csv('pi_list_japan_asym_0.4.csv',header=T)[,2:17])

pi_res = colMedians(pi_list[100:1100,])/100
#pi_res = as.vector(sapply(1:16,function(x) quantile(pi_list[100:1100,x],0.025))/100)
#pi_res = as.vector(sapply(1:16,function(x) quantile(pi_list[100:1100,x],0.975))/100)

seiq_matrix = read.csv('seiq_matrix_japan_210302.csv')[,2:6]
suscept = as.matrix(read.csv('suscept_japan_210302.csv')[,2:17])

seiq_res = seiq_matrix[seiq_matrix$E_date<=360,] # remove people who exposed after 12/25

I_total_sym_res =  matrix(0,nrow = 15 ,ncol=16) # Infectious people who exposed before 12/25
I_total_asym_res =  matrix(0,nrow = 15 ,ncol=16) # Infectious people who exposed before 12/25
for(t in 1:15){
...
for( i in 1:14){
  exposed_res[i,] = round(sapply(1:16, function(x) {suscept_res[i,x]*(pi_res[x]/(japan_age_group[x]))*(contact_matrix[x,]%*%I_total_res_meta[i,])}))
...
sum(colSums(exposed_res)) #Expected number of exposed people during 2weeks. 75464
sum(round(colSums(exposed_res)*mortality)) #Expected number of death during 2weeks. 1044
  ...
  for( iterate in 1:end){
    ...
    for( j in 1:nrow(sample_p)){
      ...
      if(max(vac_num-japan_age_group)<=0){
        ...
        suscept_res_meta[1,] = suscept[360,] - Immunity
        ...
        for( i in 1:14){
          exposed_res_meta[i,] = round(sapply(1:16, function(x) {suscept_res_meta[i,x]*(pi_res[x]/(japan_age_group[x]))*(contact_matrix[x,]%*%I_total_res_meta[i,])}))
        ...
        exposed_num[j] = sum(colSums(exposed_res_meta))
    ...
    if(max(vac_num_res-japan_age_group)<=0){
      ...
      final_num = sum(colSums(exposed_res_meta))
    }
    print(final_num)
...
suscept_res_meta = matrix(0, nrow=15, ncol=16)
suscept_res_meta[1,] = suscept[360,] - efficacy*suscept[360,]
...
num_infection_5 = c(japan_age_group,inf_5)
num_death_5 = c(japan_age_group,mor_5)
...
for(j in 1:5){
  num_infection[j,1:16] = num_infection[j,1:16]/japan_age_group
  num_death[j,1:16] = num_death[j,1:16]/japan_age_group
}
