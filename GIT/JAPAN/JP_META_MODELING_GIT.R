...
#pi_list = as.matrix(read.csv('q_list_japan_asym_0.04.csv',header=T)[,2:17])
#pi_list = as.matrix(read.csv('q_list_japan_asym_0.4.csv',header=T)[,2:17])
  ...
  for( iterate in 1:end){
    ...
    for( j in 1:nrow(sample_p)){
      ...
      if(max(vac_num-japan_age_group)<=0){
        ...
        for( i in 1:14){
          exposed_res_meta[i,] = round(sapply(1:16, function(x) {suscept_res_meta[i,x]*(q_res[x]/(japan_age_group[x]))*(contact_matrix[x,]%*%I_total_res_meta[i,])}))
