...
as.Date()
q_standard = c(2,91,153,245,289,358,nrow(corona_new))
for( i in 1:6){
...
init_q[5,] = sapply(1:16, function(x) { max(rgamma(1,0.001,rate = 0.001),1e-300)})  

q_matrix = matrix(0, nrow = nrow(corona_new),ncol = 16)
q_matrix[1,] = init_q[1,]
for( i in 1:6){
...
p_unit = function(t){
  res = (q_matrix[(t-1),]/(skage.groups_new*100)) * (contact_matrix(t-1)%*%I_total[t-1,])
  return(res)
}
...
tilde_p_unit = function(t){
  res = (q_matrix[(t-1),]/(skage.groups_new*100)) * (contact_matrix(t-1)%*%I_total_new[t-1,])
  return(res)
}
